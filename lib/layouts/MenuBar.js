...
import { Badge, Input, Button } from "lib";
    ...
    >
      {showMenu && (
        <div className="nui-menubar--container">
          <h1 className="mb-6 text-xl font-medium leading-tight text-gray-800">
            {title}
          </h1>
          {children}
        </div>
...
const SubTitle = ({ iconProps, children }) => (
  <h6 className="mb-4 text-xs font-bold tracking-wider text-gray-400 uppercase">
...
const Block = ({ label, icon, count, active, onClick, onEdit, url }) => {
  const handleEdit = e => {
    e.stopPropagation();
    onEdit();
  };
