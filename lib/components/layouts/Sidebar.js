...
export const ProfileSectionTooltip = ({profileInfo}) => {
  ...
  return (
    <div className="flex flex-col p-6">
      <div className="relative pl-12 mb-5">
        ...
        <div className="flex flex-col flex-grow pl-1">
          ...
          <Typography style="body2" className="w-full truncate neeto-ui-text-gray-600" lineHeight="tight">
            jordan.moyas@company.com
          </Typography>
        </div>
      </div>
      {
        profileInfo.dropdownProps && (
          <ul className="flex flex-col gap-2 py-2 list-none border-t neeto-ui-border-gray-300">
            {profileInfo.dropdownProps?.map(({ onClick, label, icon = null, ...otherProps }, idx) => {
              const IconSVG = icon;
              return (
                <li key={idx} onClick={onClick} {...otherProps} className="p-2 transition-colors rounded-sm cursor-pointer hover:neeto-ui-bg-gray-200">
                  <Typography
                    style="h5"
                    weight="semibold"
      ...
      {
        profileInfo.bottomListProps && (
          <ul className="flex flex-col gap-2 pt-2 list-none border-t neeto-ui-border-gray-300">
            {profileInfo.bottomListProps?.map(({ onClick, label, icon, ...otherProps }, idx) => {
              const IconSVG = icon;
              return (
                <li key={idx} onClick={onClick} {...otherProps} className="p-2 transition-colors rounded-sm cursor-pointer hover:neeto-ui-bg-gray-200">
                  <Typography
...
export const ProfileSection = forwardRef(({ collapsed, profileInfo, onClick }, ref) => {
  const truncate = (str, length) => {
    return str && str.length > length
      ? str.substring(0, length - 3) + "..."
      : str;
  ...
  return (
    <div ref={ref} className="neeto-ui-sidebar__profile-wrapper" onClick={onClick} data-cy={dataCy}>
...
export const NavIconWrapper = ({ tooltipStyle, description, icon, collapsed, mainLabel, children }) => {
  const tooltipContent = tooltipStyle === TOOLTIP_STYLES.featured ? (
    <FeaturedTooltip description={description} icon={icon} mainLabel={mainLabel} />
  ) : mainLabel;
  const content = collapsed ? (
    <Tooltip position="right" distance="20rem" content={tooltipContent} theme={tooltipStyle === TOOLTIP_STYLES.featured ? "light" : "dark"} className={ tooltipStyle === TOOLTIP_STYLES.featured ? "sidebar-featured-tooltip__content" : ""}>
      {children}
    </Tooltip>
  ) : (
    children
  ...
  return (
    ...
    >
      ...
      <div className="neeto-ui-sidebar__links">
        {navLinks.map(
          (
            { label: mainLabel, to: mainRoute, icon, description, items, ...otherProps },
            mainIndex
          ) => {
            const IconSVG = icon;
            ...
            return (
              <React.Fragment key={mainIndex}>
                ...
                >
                  {icon && (
                    <NavIconWrapper tooltipStyle={tooltipStyle} icon={<IconSVG color="#68737D" />} description={description} collapsed={collapsed} mainLabel={mainLabel}>
      ...
      <div className="neeto-ui-sidebar__footer">
        ...
        {showAppSwitcher && (
          ...
          >
            <NavIconWrapper collapsed={collapsed} mainLabel="App Switcher">
        ...
        {/* {profileInfo && (
          ...
          >
            {profileInfo.dropdownProps?.map(({ onClick, label, ...otherProps }, idx) => (
        ...
        {profileInfo && (
          <Tooltip position="right" className="sidebar-featured-tooltip__content" theme="light" content={<ProfileSectionTooltip profileInfo={profileInfo}/>} interactive	
          >
            <ProfileSection
              collapsed={collapsed}
              profileInfo={profileInfo}
