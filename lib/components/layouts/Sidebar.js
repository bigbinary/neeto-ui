...
export const ProfileSection = ({ collapsed, profileInfo, onClick }) => {
  ...
  return (
    <div className="neeto-ui-sidebar__profile-wrapper" onClick={onClick}>
      <div className="flex items-center flex-shrink-0 neeto-ui-sidebar__profile">
        ...
        {!collapsed && (
          <div className="neeto-ui-sidebar__profile-content">
            ...
            <Down size={16} className="neeto-ui-sidebar__profile-drop-icon" color="#68737D" />
          </div>
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
          ...
          (
            ...
            return (
              <React.Fragment key={mainIndex}>
                <NavLink
                  to={shouldOpenSidebar && isActive ? location.pathname : items ? items[0].to : mainRoute}
                  className="flex items-center no-underline select-none neeto-ui-sidebar__link"
                  activeClassName="active"
                  isActive={() => isActive}
                  onClick={() => shouldOpenSidebar && setCollapsed(false)}
                  {...otherProps}
                >
                  {icon && (
      ...
      <div className="neeto-ui-sidebar__footer">
        ...
        {showChangelog && (
          <div className="neeto-ui-sidebar__links">
            <div className="flex items-center no-underline select-none neeto-ui-sidebar__link">
              <Rating />
              {!collapsed && (
                <Typography
                  component="span"
        ...
        {showAppSwitcher && (
          <div className="neeto-ui-sidebar__links mb-4">
            <div className="flex items-center justify-center">
              <Button
                onClick={onAppSwitcherToggle}
                icon={() => <AppSwitcher size={40} color="#68737D" />}
                size="small"
                label={
                  !collapsed && (
                    <div className="flex items-center mx-4 space-x-4">
                      <Typography lineHeight="tight" style="h5" weight="semibold" className="neeto-ui-text-gray-600">
                        {appName}
                      </Typography>
                      <Up size={16} color="#87929d" />
                    </div>
                  )
                }
                style="text"
                iconPosition="left"
                iconSize={24}
                className="flex mx-auto neeto-ui-sidebar__collapse-btn"
              />
            </div>
          </div>
        )}
        ...
        {collapsible && (
          <div className="flex items-center w-full neeto-ui-sidebar__collapse-wrap">
            <Button
              onClick={() => setCollapsed(!collapsed)}
              size="small"
              label={!collapsed ? "Collapse" : ""}
              style="text"
              icon={!collapsed ? LeftArrow : RightArrow}
              iconPosition="left"
              iconSize={24}
              className="flex mx-auto neeto-ui-sidebar__collapse-btn"
            />
...
Sidebar.propTypes = {
  ...
  toggleAppSwitcher: PropTypes.bool,
