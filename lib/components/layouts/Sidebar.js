...
const Sidebar = ({
  ...
  const { icon: ChangelogIcon, ...updatedChangelogProps } = changelogProps;
  ...
  return (
    ...
    >
      ...
      <div className="neeto-ui-sidebar__links">
        {navLinks.map(
          (
            ...
            const IconSVG = icon;
            const isActive = items?.length ? items.some(({ to }) => location.pathname.includes(to)) : location.pathname.includes(mainRoute);
      ...
      <div className="neeto-ui-sidebar__footer">
        ...
        {showAppSwitcher && (
          <div className="mb-4 neeto-ui-sidebar__links">
