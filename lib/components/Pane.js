    ...
    <Portal className="neeto-ui-portal">
      <AnimatePresence>
        {isOpen && (
          <Backdrop
            key="pane-backdrop"
            className={classnames("neeto-ui-pane__backdrop", backdropClassName)}
          >
            <motion.div
              initial={{
                x: PANE_SIZE,
...
const Header = ({ children, className }) => {
  return (
    <div className={classnames("neeto-ui-pane__header", className)}>{children}</div>
  );
};
...
const Body = ({ children, className, hasFooter = true }) => {
  return (
    <div
      className={classnames("neeto-ui-pane__body", {
...
const Footer = ({ children, className }) => {
  return (
    <div className={classnames("neeto-ui-pane__footer", className)}>{children}</div>
  );
};
