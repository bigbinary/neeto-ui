...
const Pagination = ({ count = 0, pageNo, pageSize, navigate, className = "" }) => {
  let start = 0, end = 0;
  if (count) {
    start = pageNo > 1 ? (pageNo - 1) * pageSize + 1 : 1;
    end = count < pageNo * pageSize ? count : pageNo * pageSize;
  }
  const isLastPage = pageNo * pageSize === count;

  const jumpToLast = () => {
    const lastPageIndex = Math.floor(count / pageSize);
    navigate(lastPageIndex);
  };

  const baseClasses = [
    "relative inline-flex items-center px-2 py-2 text-sm font-medium leading-5 text-gray-400",
    "transition duration-150 ease-in-out bg-white border border-gray-300 focus:outline-none"
  ];

  const psuedoClasses = "hover:bg-gray-50 hover:text-gray-700 active:bg-gray-100 active:text-gray-700";
  const disabledClasses = "cursor-not-allowed bg-gray-100";

  const disableNavigateBack = count === 0 || pageNo === 1;
  const disableNavigateForward = count === 0 || isLastPage;
  ...
  return (
    ...
    >
      <Dropdown
        ...
        autoWidth
      >
        <li onClick={() => count !== 0 && navigate(1)}>Jump to first</li>
        <li onClick={() => count !== 0 && jumpToLast()}>Jump to last</li>
      </Dropdown>
      <div className="relative z-0 inline-flex ml-3 shadow-sm">
        <button
          className={classnames(baseClasses, "rounded-l-md border-r-0", {
            [psuedoClasses]: !disableNavigateBack,
        ...
        >
          <i className="align-middle ri-arrow-left-s-line ri-lg"></i>
        </button>
        <button
          className={classnames(baseClasses, "rounded-r-md", {
            [psuedoClasses]: !disableNavigateForward,
        ...
        >
          <i className="align-middle ri-arrow-right-s-line ri-lg"></i>
        </button>
      </div>
    </div>
  );
