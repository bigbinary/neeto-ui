...
const Card = ({ children, className = "", rows = null }) => {
  ...
  const renderChildren = () => {
    if (shouldCollapse) {
      const nonCollapsibleItems = children[1].slice
        ? children[1].slice(0, rows)
        : reactChildren[1].slice(0, rows);
      const collapsibleItems = children[1].slice
        ? children[1].slice(rows)
        : reactChildren[1].slice(rows);
      return (
        <>
          {/* Card title */}
          {children[0]}
