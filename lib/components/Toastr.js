...
const ToastrComponent = ({ type, message, buttonLabel = "", onClick = noop }) => {
  let icon;
  switch (type) {
    case "success":
...
const showToastr = (message, buttonLabel, onClick) => {
  toast.success(<ToastrComponent type="success" message={message} buttonLabel={buttonLabel} onClick={onClick} />, {
...
const showInfoToastr = (message, buttonLabel, onClick) => {
  toast.info(<ToastrComponent type="info" message={message} buttonLabel={buttonLabel} onClick={onClick} />, {
    position: toast.POSITION.BOTTOM_CENTER,
    transition: Slide,
  });
};
...
const showErrorToastr = (errorObject, buttonLabel, onClick) => {
  ...
  toast.error(<ToastrComponent type="error" message={errorMessage} buttonLabel={buttonLabel} onClick={onClick} />, {
    position: toast.POSITION.BOTTOM_CENTER,
    transition: Slide,
    autoClose: 10000,
  });
