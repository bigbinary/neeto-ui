...
const EmailInput = ({
  ...
  const renderValidEmails = () => value.filter(email => email.valid && email);

  const handleFilterEmails = () => onChange(renderValidEmails());
  ...
  return (
    <div className="flex flex-col">
      ...
      <div>
        {!!error && (
          <Typography style="body3" className="flex neeto-ui-text-error mt-1">
            {error}
...
EmailInput.propTypes = {
   ...
   */
  filterInvalidEmails: PropTypes.shape({ label: "click here" }),
