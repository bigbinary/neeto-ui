...
const getToastrMessage = (message) => {
  if (typeof message === "object" && message.notice_code)
    return t(message.notice_code, message.entity_name);
...
const withUniqueCheck = (type, toastFunc) => (message, ...toastrConfig) => {
  ...
  if (
    toastrList.add({ type, message: getToastrMessage(message), buttonLabel })
  ) {
    const config = {
      ...TOAST_CONFIG,
