...
const getToastrMessage = (message) => {
  if (typeof message === "object" && message.noticeCode)
...
const getErrorMessage = response => {
  ...
  const errorCodeTranslations = (errorCode && t(errorCode, response)) ||
      (errorCodes && errorCodes.map(errorCode =>
        errorCodeTranslation(errorCode)).join("\n")) || "";
...
const withParsedErrorMsg = (toastrFunc) => (errorObject, ...toastrConfig) => {
  ...
  if (errorObject?.response?.data?.noticeCode) {
