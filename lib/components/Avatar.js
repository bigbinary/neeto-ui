...
const Avatar = ({
    ...
    return (
      <span
        className={classnames({
          "absolute top-0 z-5 block w-2 h-2 text-white transform translate-x-1/2 rounded-full right-1 shadow-solid": status,
  ...
  const Activity = () => {
    ...
    return (
      <div className="absolute bottom-0 right-0 flex items-center justify-center w-5 h-5 text-sm text-gray-700 transform translate-x-1 translate-y-2 bg-gray-100 rounded-full shadow-md bsolute">
  ...
  const renderUserInitial = (userName) => {
    ...
    const charCode =
      (userInitial.charCodeAt(0) || 0) + (userInitial.charCodeAt(1) || 0);
    const bgColor =
    ...
    return (
      <div
        className={classnames([
          ...
          bgClassName ? bgClassName : backgroundClass,
          className,
        ])}
        style={style}
        {...otherProps}
  ...
  const renderUserImage = (profileImagePath) => {
    return (
      <div
        className={classnames(["relative", className, bgClassName])}
        style={style}
        {...otherProps}
      >
        <img
  ...
  const renderDefaultAvatar = () => {
    return (
      <div
        className={classnames([
          "relative text-white rounded-full text-center",
          className,
          bgClassName ? bgClassName : "bg-purple-400",
        ])}
        style={style}
        {...otherProps}
      >
  ...
  if (
    either(isNil, isEmpty)(contact) ||
    all(either(isNil, isEmpty), [contact.name, contact.profile_name])
  ) {
